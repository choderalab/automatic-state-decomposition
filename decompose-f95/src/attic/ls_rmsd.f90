...
!      UCSF, University of New Mexico, Seoul National University
...
MODULE ls_rmsd
!-----------------------------------------------------------------------
  use numeric_kinds ! for precision definitions
...
CONTAINS
!-----------------------------------------------------------------------
function rmsd(n, coord1, coord2)
!-----------------------------------------------------------------------
!  This function returns the least square rmsd of two coordinate
!  sets coord1(3,n) and coord2(3,n) using a method based on quaternion.
!-----------------------------------------------------------------------

  ! Parameters.
  real(dp) :: rmsd ! return value -- LS-RMSD between point sets
  integer, intent(in) :: n ! number of points
  real(dp), dimension(3,n), intent(in) :: coord1, coord2 ! coordinate sets, 3xn
  ...
  ! RMS Deviation
  rmsd = sqrt(max(0.0d0,((x_norm+y_norm)-2.0d0*lambda))/dble(n))

end function rmsd
!-----------------------------------------------------------------------
subroutine rmsd_and_gradient(n, coord1, coord2, option, U, x_center, y_center, & 
     error, calc_g, g)
!-----------------------------------------------------------------------
...
end subroutine rmsd_and_gradient
!-----------------------------------------------------------------------
