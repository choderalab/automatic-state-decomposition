...
contains
  ...
  function ls_rmsd(N, x_original_nk, y_original_nk)
    ...
    x_centroid_k = sum(x_original_nk(1:N,1:3),1) / real(N,dp)
    y_centroid_k = sum(y_original_nk(1:N,1:3),1) / real(N,dp)
...
!    if(.not. (ls_rmsd <= 0 .or. ls_rmsd > 0)) then
  ...
  subroutine compute_rotation_matrix(N, x_nk, y_nk, U)
    ...
    ! additional precision was needed to obtain an accurate R that does not have negative
    ! eigenvalues.
    R = matmul(transpose(real(y_nk(1:N,1:3),dp)), real(x_nk(1:N,1:3),dp))
  ...
  subroutine ls_align(N, x_nk, y_target_nk, atom_indices)
    ...
    integer, intent(in)                       :: N
      ! The number of vectors in the set to be aligned.
    real(sp), dimension(:,:), intent(inout) :: x_nk
      ! Coordinate set to be rotated and translated.
      ! x_nk(n,k) is the kth dimension (k=1..3) of atom n (n=1..N)    
    real(sp), dimension(:,:), intent(in) :: y_target_nk
      ! Coordinate set for which x_nk is to be aligned to.
    ...
    if(present(atom_indices)) then
       ...
       x_centroid_k(1:3) = sum(x_nk(atom_indices,1:3),1) / real(nindices,sp)
       y_centroid_k(1:3) = sum(y_target_nk(atom_indices,1:3),1) / real(nindices,sp)       
    else
       x_centroid_k(1:3) = sum(x_nk(1:N,1:3),1) / real(N,sp)
       y_centroid_k(1:3) = sum(y_target_nk(1:N,1:3),1) / real(N,sp)
    end if

    ! Translate x and y to their centroids.
    forall(nIndex = 1:N)
       x_nk(nIndex,1:3) = x_nk(nIndex,1:3) - x_centroid_k(1:3)
       y_nk(nIndex,1:3) = y_target_nk(nIndex,1:3) - y_centroid_k(1:3)
    end forall

    ! Compute rotation matrix for optimal LS-alignment using atom list.
    if(present(atom_indices)) then
       call compute_rotation_matrix(nindices, x_nk(atom_indices,1:3), y_nk(atom_indices,1:3), U)
    else 
       call compute_rotation_matrix(N, x_nk(1:N,1:3), y_nk(1:N,1:3), U)
    end if
    
    ! Apply U to x and transform to centroid of y.
    forall(nIndex = 1:N)
       x_nk(nIndex,1:3) = matmul(U, x_nk(nIndex,1:3)) + y_centroid_k(1:3)
    end forall
